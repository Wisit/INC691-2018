...
void ClientRequested(void *evt)
{
    ...
    if( !strcmp( client->getBuffer, "led0-inv" ) ) 
    {
        ...
        const char *class  = LED0_Get() ? "danger" : "default";
        sprintf(pPage, "<h1 class='text-%s'>LED0: %s</h1>", class, status);
        client->data = pPage;
    }
    if( !strcmp( client->getBuffer, "adc1" ) ) 
    {
        char *pPage = malloc(128);
        sprintf(pPage, "<h1 class='text-success'>ADC1: %1.3f Volts</h1>", ADC1_Get()*3.3/1023.0);
        client->data = pPage;
    }
    Beep(50);
}
