...
void Server_Service(void *evt)
{
    ...
    if( client->state == CLIENT_STATE_REQUESTED ) 
    {
        ...
        if(client->data == NULL)
        {
            ...
            client->data = "<h1 class='text-danger'>Page Not Found!!</h1>";
        }
        ...
        if( strlen( client->data ) > 2048 )  
        {
            ...
            client->data = "<h1 class='text-danger'>The payload is greater than 2048 bytes!</h1>";
        }
        ...
        AT_Put( pCIPSEND ); //!! AT_Put allocates memory and copy the command to that buffer
        free(pCIPSEND);
    }
